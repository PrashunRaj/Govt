...
// Helper function to format budget with appropriate denomination
...
const Heatmap = () => {
  ...
  }, [filter, categoryFilter, userProfile]);
  ...
  const fetchProposals = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        // Auto-center map based on proposals
  ...
  const centerMapOnProposals = (proposalList) => {
    ...
    setMapZoom(12); // Zoom closer since it's only one constituency
  };

  // Process data to get proposal density by geographic clusters
  const processGeographicClusters = () => {
    const clusters = [];
    const clusterRadius = 0.1; // Smaller radius since it's only one constituency
    ...
    clusters.forEach(cluster => {
      if (cluster.count >= 5) cluster.density = 'high';
      else if (cluster.count >= 3) cluster.density = 'medium';
  ...
  const filteredProposals = proposals.filter(proposal => {
    ...
    if (searchQuery && !proposal.title.toLowerCase().includes(searchQuery.toLowerCase()) &&
        !proposal.description.toLowerCase().includes(searchQuery.toLowerCase())) return false;
    return true;
  });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Please complete your profile to view the proposal heatmap.</p>
        </div>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">My Constituency Heatmap</h1>
            <p className="text-gray-600 mt-1">
              Proposals from {userProfile.constituency} - {proposals.length} total proposals
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden map-container-wrapper">
              <div style={{ 
                height: '600px', 
                width: '100%', 
                position: 'relative',
                ...
                >
                  ...
                  {showDensityCircles && geographicClusters.map((cluster, index) => (
                    ...
                    >
                      <Popup>
                        <div className="p-2">
                          <h3 className="font-semibold text-gray-900">Local Cluster</h3>
                          <p className="text-sm text-gray-600">{cluster.count} proposal(s)</p>
                          <p className="text-sm text-gray-600">
                            Total Budget: {formatBudget(cluster.totalBudget)}
                          </p>
                      ...
                      <Popup>
                        <div className="p-3 max-w-sm">
                          ...
                          <div className="space-y-1 text-sm text-gray-600">
                            ...
                            <p><strong>Budget:</strong> {formatBudget(proposal.budget || 0)}</p>
                            <p><strong>Votes:</strong> {proposal.votes || 0}</p>
                            <p><strong>Submitted by:</strong> {proposal.submittedBy}</p>
                            <p><strong>Date:</strong> {proposal.submittedDate}</p>
                          </div>
                ...
                {/* Legend */}
                {showLegend && (
                  <div className="map-legend top-4 right-4 bg-white rounded-lg shadow-lg border border-gray-200 p-4">
                    ...
                    <div className="space-y-3">
                      ...
                      {showDensityCircles && (
                        <div>
                          <h4 className="text-sm font-medium text-gray-700 mb-2">Local Density</h4>
                          <div className="space-y-1">
                            <div className="flex items-center space-x-2">
                              <div className="w-4 h-4 rounded-full bg-red-500 opacity-30"></div>
                              <span className="text-xs text-gray-600">High (5+ proposals)</span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <div className="w-4 h-4 rounded-full bg-yellow-500 opacity-30"></div>
                              <span className="text-xs text-gray-600">Medium (3-4 proposals)</span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <div className="w-4 h-4 rounded-full bg-green-500 opacity-30"></div>
                              <span className="text-xs text-gray-600">Low (1-2 proposals)</span>
                            </div>
                          </div>
                        </div>
                      )}
          ...
          {/* Sidebar */}
          <div className="space-y-6">
            {/* User Constituency Info */}
            {userProfile && (
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">My Constituency</h3>
                <div className="space-y-2">
                  <p className="text-sm text-gray-600">
                    <strong>Constituency:</strong> {userProfile.constituency}
                  </p>
                  <p className="text-sm text-gray-600">
                    <strong>District:</strong> {userProfile.district}
                  </p>
                  <p className="text-sm text-gray-600">
            ...
            {/* Local Statistics */}
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Local Statistics</h3>
              <div className="space-y-4">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-600">Local Clusters</span>
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="text-sm text-gray-600 space-y-1">
                    ...
                    <p><strong>Budget:</strong> {formatBudget(selectedProposal.budget || 0)}</p>
                    <p><strong>Votes:</strong> {selectedProposal.votes || 0}</p>
                    <p><strong>Submitted by:</strong> {selectedProposal.submittedBy}</p>
                    <p><strong>Date:</strong> {selectedProposal.submittedDate}</p>
                  </div>
